...
| Kit that helps maintainers generate and validate a compliant control plane. Lead name is **TCF**, not “Toolchain Management Framework” (TMF)—TMF sounds like “only if I’m building a management app.” See xref:explanation/infrastructure/toolchain-framework.adoc[Toolchain Framework (TCF)] and xref:toolchain-framework::index.adoc[the TCF practice kit].
|===
...
* xref:toolchain-framework::index.adoc[TCF practice kit]
* link:https://github.com/dev-centr/toolchain-framework[dev-centr/toolchain-framework]
* xref:explanation/infrastructure/os-install-concurrency.adoc[OS install concurrency] — when agents race host package installs during bootstrap
