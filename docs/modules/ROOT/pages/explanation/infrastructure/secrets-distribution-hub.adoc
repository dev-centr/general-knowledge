...
Name the credential by architecture before you distribute it: xref:explanation/infrastructure/api-key-vs-token.adoc[API key vs token] (long-lived project keys and PATs are still keys; short-lived handshake credentials are access tokens).
...
See also xref:explanation/infrastructure/composio-and-mcp-connectors.adoc[Composio and MCP connectors] and xref:explanation/infrastructure/studio-mcp-install-strategies.adoc[Studio MCP install strategies].

== Pulumi ESC (and friends)

xref:explanation/infrastructure/iac.adoc[Infrastructure as Code] can own *distribution*:
