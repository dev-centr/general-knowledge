...
:description: Architectural framework for shippable software—shared essentials (About/build info, updates, packaging, pipelines, diagnostics), auxiliary feature templates for desktop/web/mobile, and mappings by artifact type.
...
[[auxiliary-and-classes]]
== Auxiliary features and delivery classes

Beyond the shared essentials table, every **end-user shell** should plan a deliberate set of xref:explanation/architecture/auxiliary-features.adoc[auxiliary features] (Help, What's new, Preferences, connectivity, privacy, support path, …).

Delivery class templates (copy into the project plan):

* xref:explanation/architecture/classes/index.adoc[Delivery classes overview]
* xref:explanation/architecture/classes/desktop.adoc[Desktop] — Help/About, tray, autostart, OS-specific update apply (including quit/replace on Unix)
* xref:explanation/architecture/classes/web.adoc[Web] — build identity, deploy-as-update, PWA reload, status/privacy
* xref:explanation/architecture/classes/mobile.adoc[Mobile] — store updates, permissions, crash export, policy-required account deletion
