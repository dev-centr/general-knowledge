...
| **SDLang (`.sdl`)**
| DevCentr owns the parser (catalogs, `.devcentr.sdl`, recognizer profiles). Comments, native types, low punctuation noise. See xref:DevCentr:specifications:sdl.adoc[SDL Preference].

| **KDL (`.kdl`)**
| Modern node-based successor to SDLang: strict grammar, `/-` node comments, `(type)` annotations—and the consumer already speaks KDL. Not the DevCentr-owned default while sdlang-d remains the stack.
...
Both are node-based documents. KDL was shaped as a clearer successor to SDLang (unambiguous grammar, explicit type annotations, safer node commenting). DevCentr still **favors SDLang** for files it parses today; evaluate KDL when starting greenfield config outside that parser surface.
