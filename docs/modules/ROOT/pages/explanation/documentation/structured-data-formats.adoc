...
| **SDLang (`.sdl`)**
| DevCentr owns the parser (catalogs, `.devcentr.sdl`, recognizer profiles, CentrMark properties). Comments, native types, low punctuation noise. See xref:platforms:specifications:sdl.adoc[SDL Preference].
...
Both SDLang and KDL are node-based documents. KDL is the clearer modern successor (unambiguous grammar, explicit type annotations, safer node commenting). **Policy:** keep SDLang where DevCentr already parses `.sdl`; prefer KDL for new documents outside that surface. Do **not** adopt Extended SDL/XDL as a compromise—it does not finish SDLang’s gaps and does not inherit KDL’s ecosystem. Full decision table: xref:platforms:specifications:sdl.adoc#decision-sdlang-vs-kdl-vs-extended-sdl[SDL Preference — Decision].
...
* Create… catalog and dialog: xref:platforms:specifications:components/content-create.adoc[Content Create]
* Prose markup: xref:explanation/documentation/documentation-markup-comparison.adoc[Documentation markup]
* SDL policy: xref:platforms:specifications:sdl.adoc[SDL Preference]
