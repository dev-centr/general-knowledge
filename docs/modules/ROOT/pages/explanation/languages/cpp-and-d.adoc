...
* GC when you want to think about the problem; `@nogc` / `-betterC` when you need to think about the heap. See xref:explanation/languages/d-and-rust.adoc[D and Rust] for the honest cost of that split.
...
* Not a claim that D's GUI ecosystem matches those incumbents. Pair D (or Rust) in the host with a webview UI when that is the product; see xref:home::explanation/architecture/ui-heavy-web-apps.adoc[UI-heavy web applications].
