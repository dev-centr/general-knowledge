...
DevCentr treats this as Internet Architecture (systems altitude): shared-memory durability for correspondence and structured collections, not only packet uptime. HCI Nerdz owns the human symptom (*host-held history*). connectome-fs owns substrate instincts (portable identity of content). Issues Browser + personal `.issues` are the **near-term demonstration wedge**.
