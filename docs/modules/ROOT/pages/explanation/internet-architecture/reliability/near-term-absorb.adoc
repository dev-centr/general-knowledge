= Near-term absorb
:description: Redirects, aliases, Cool URIs, and CONSUMERS.md—pragmatic absorb while labels still wire the web.
:page-audience: Docs maintainers and platform engineers buying time with redirects, aliases, and Cool URI discipline
:page-usage-context: Encyclopedia explanation under Internet Reliability; pragmatic absorb while labels still wire the web
:page-orig-author: Ryan Johnson
:page-last-author: Cursor agent on behalf of Ryan Johnson
:page-last-edited: 2026-09-15

ifndef::page-context-active[]
[.page-context.page-context-lead]
****
Audience:: {page-audience}
****

[.page-context.page-context-footer]
****
Original author:: {page-orig-author}
Latest contributor:: {page-last-author}
Last updated:: {page-last-edited}
****
endif::[]


Until the world stops wiring on strings, Internet Reliability needs an **absorb layer**.
These practices do not replace identity; they buy time and make renames searchable.

[source,mermaid]
----
flowchart LR
  Old[Old public label]
  Alias[Alias / 301 / Antora page alias]
  New[New human path]
  Id[Stable id / xref hub]
  Old --> Alias --> New
  Alias -.-> Id
  New -.-> Id
----

== Practices that earn their keep

[cols="1,2"]
|===
| Practice | Why it helps

| Cool URI discipline (W3C)
| Prefer not to change public paths; treat published URLs as contracts

| Redirects (301) and vanity hosts
| Absorb inevitable moves without orphaning bookmarks and citations

| Antora page aliases / unversioned component URLs
| Keep `/component/` and renamed pages resolving on docs hubs

| `CONSUMERS.md` (or `REVERSE_DEPENDENCIES.md`)
| Producer-owned inbound list: who hardcodes this path, URL, or slug

| Hub `xref:` over raw foreign URLs
| Cross-component links survive playbook moves better than pasted HTTPS
|===

== `CONSUMERS.md` shape (orientation)

Own the list on the *producer*. Record coupling type (package import vs URL vs Antora component). Prefer aliases over renames. Teach agents: read consumers → search those trees → open PRs.

Full rules and examples: link:https://hci-nerdz.github.io/docs/hci-nerdz/an-alternative-to-urls.html[HCI — Labels versus wires] (Treatment section).

== Related

* xref:explanation/internet-architecture/reliability/identity-stack.adoc[Identity stack]
* xref:home::publishing/antora-essential-extensions.adoc[Antora essential extensions] — alias-to-latest and related plumbing
* link:https://www.w3.org/Provider/Style/URI[Cool URIs don't change]
