...
SLP tangles **source**. For docs you **play on the machine** (install steps, DNS checks, setup recipes), use link:https://github.com/dev-centr/scriptbook[Scriptbook]: CentrMark `::: playbook` / `::: step` with run output in sidecars (never rewritten into the `.cmk`). Shell backends first; other runners later.
