...
=== Runnable script documents (Scriptbook)
SLP tangles **source**. For docs you **play on the machine** (install steps, DNS checks, setup recipes), use link:https://github.com/dev-centr/scriptbook[Scriptbook]: CentrMark `::: playbook` / `::: step` with run output in sidecars (never rewritten into the `.cmk`). Author **intents**, not a bash-to-PowerShell rewrite. Illustrated model: xref:explanation/infrastructure/abstract-playbooks.adoc[Abstract playbooks].
...
* link:https://github.com/dev-centr/scriptbook[Scriptbook] — playable `.cmk` script documents
