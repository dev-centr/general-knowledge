...
:description: Put Cloudflare Access in front of a static member site, choose an identity provider, and add people without creating Cloudflare dashboard users. Includes simplified mocks of the GitHub OAuth app, Zero Trust IdP, Allow policy, and login page.
...
Figures on this page are **simplified mocks** (field labels from Cloudflare / GitHub docs as of 2026-08). They are not live dashboard captures. Secrets are drawn as bullets.
...
+
`<team-name>` is the Zero Trust team name (**Zero Trust** → **Settings** → **Team name**). This is **not** the docs hostname.
...
Until Access is on, the origin is world-readable even if the git repo is private. Treat the hostname as unpublished.

== Related

* xref:explanation/infrastructure/access-in-front-of-static-sites.adoc[Access in front of static sites] — why the lock is at the edge; xref:explanation/infrastructure/access-in-front-of-static-sites.adoc#how-github-org-membership[how GitHub org membership is checked]
